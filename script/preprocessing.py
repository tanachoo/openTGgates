...
def main():
    ...
    att_ = att.loc[:, ['BARCODE', 'COMPOUND_NAME', 'COMPOUND Abbr.', 'SPECIES', 'SACRI_PERIOD', 'DOSE_LEVEL']]
    ...
    labellist=[]
    for k, l, m in zip(att_['COMPOUND Abbr.'], att_['SACRI_PERIOD'], att_['DOSE_LEVEL']):
        compound = k
        time = l.replace(' ', '')
        dose = m
        label = compound + '_' + time + '_' + dose
    ...
    att_ = att_[(att_['COMPOUND Abbr.'] == 'APAP') | (att_['COMPOUND Abbr.'] == 'CCL4')]
    #att_ = att_[(att_['COMPOUND Abbr.'] == 'APAP')]
    ...
    file2 = '../data/Human_in_vitro/*/celfiles/*csv'
    ...
    print(f'[SAVE]: ../data/matrix_TEST.txt')
    with open('../data/matrix_TEST.txt', 'w') as f:
